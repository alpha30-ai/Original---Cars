...
import { CheckCircle2, Hash, Mail, ArrowRight, ShieldCheck, Sparkles, ChevronLeft } from "lucide-react";
...
function VerifyForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(data.message || "تم تفعيل حسابك بنجاح! يمكنك الآن تسجيل الدخول.");
        router.push("/login");
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1.5">
        <label className="block text-xs font-bold text-foreground">كود التفعيل (6 أرقام)</label>
        <div className="relative">
          ...
          <input
            ...
            required
            maxLength={6}
            className="w-full bg-background border border-border rounded-2xl py-3.5 pl-4 pr-11 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50 text-center tracking-[0.5em] font-mono text-xl font-black"
            value={code}
    ...
    <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 lg:p-8 bg-background relative overflow-hidden" dir="rtl">
      ...
      <div className="w-full max-w-lg relative z-10 bg-card/95 backdrop-blur-2xl rounded-3xl border border-border shadow-2xl p-8 sm:p-12">
        <div className="text-center mb-8">
          ...
          <div className="w-14 h-14 bg-emerald-500/10 text-emerald-500 rounded-2xl flex items-center justify-center mx-auto mb-3 border border-emerald-500/20">
            <CheckCircle2 className="w-7 h-7" />
          </div>
          <h1 className="text-2xl font-black text-foreground">تفعيل الحساب</h1>
          <p className="text-xs text-muted-foreground mt-1">أدخل الكود المكون من 6 أرقام المرسل إلى بريدك الإلكتروني</p>
        </div>
